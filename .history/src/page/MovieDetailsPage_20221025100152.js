...
const Trailer = () => {
  ...
  return <>
    {trailer.length > 0 && trailer.slice(0,1).map((item) => (
      ...
      <iframe width="1024" height="480" src={`https://www.youtube.com/embed/${item.key}`} title={item.name} frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullscreen></iframe>
      </div>
    ))}
  </>;
};
