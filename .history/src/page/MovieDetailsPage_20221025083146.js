    ...
    <MovieDetailsPageStyles>
      ...
      <div className="container main">
        ...
        <div className="right">
          ...
          <div className="btn-show">Show more</div>
        </div>
      </div>
    </MovieDetailsPageStyles>
  );
      ...
      {cast.length > 0 &&
        cast.slice(0, 4).map((item) => (
          <div className="card" key={item.id}>
            <img
              src={`https://image.tmdb.org/t/p/original/${item.profile}`}
              alt=""
            />
            <div className="content-right">
              <p className="cast_title">{name}</p>
              <span className="cast_add">{character}</span>
            </div>
          </div>
        ))}
    </>
