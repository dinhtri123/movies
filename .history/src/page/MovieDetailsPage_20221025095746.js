...
const Trailer = () => {
  ...
  return <>
    {trailer.length > 0 && trailer.slice(0,1).map((item) => (
      <div className="show_trailer" key={item.id}>
      <iframe width="1024" height="480" src=`https://www.youtube.com/embed/mSfsGTIQlxg`} title="How To Make Circular Progress Bar Using HTML CSS JavaScript | Neomorphism Progress Bar" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
      </div>
    ))}
  </>;
};
